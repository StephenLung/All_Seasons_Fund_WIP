...
factors_input_2 <- "Developed_3_Factors"
...
# Model Lime Single Feature Explanation ----

portfolio_automl_models_h2o@leaderboard

automl_first_model <- portfolio_automl_models_h2o@leaderboard %>% 
    as.tibble() %>% 
    select(model_id) %>% 
    slice(1) %>% 
    pull(model_id) %>% 
...
# Model Lime Multiple Feature Explanation ----
explanation <- test_tbl %>%
    slice(1:20) %>%
    select(-Portfolio) %>%
    lime::explain(
...
# Testing on Fama French Factors only ---- [TESTING]
FF_train_tbl <- Port_FF_5_tbl %>% 
    filter(date < as.Date("2019-01-01")) 

FF_test_tbl <- Port_FF_5_tbl %>% 
    filter(date >= as.Date("2019-01-01"))

FF_train_tbl_h2o <- as.h2o(FF_train_tbl)
FF_test_tbl_h2o <- as.h2o(FF_test_tbl)

y <- "Portfolio"
x <- setdiff(names(FF_train_tbl_h2o), y)

FF_portfolio_automl_models_h2o <- h2o.automl(
    x = x,
    y =y,
    training_frame = FF_train_tbl_h2o,
    max_runtime_secs = 60,
    stopping_metric = "deviance")

FF_portfolio_automl_models_h2o@leaderboard
h2o.getModel("GBM_grid_1_AutoML_20200120_174352_model_83")
pacman::p_load("lime")

explainer <- FF_train_tbl %>%
    select(-Portfolio, -date) %>% 
    lime(
        model = h2o.getModel("GBM_grid_1_AutoML_20200120_174352_model_83")
    )
explanation <- FF_test_tbl %>% 
    select(-Portfolio, -date) %>% 
    lime::explain(
        explainer = explainer,
        n_labels   = 1,
