...
multi_asset_return_portfolio <- function(stock_price_tbl, period = "monthly"){
    ...
    # account for daily and weekly inputs which do not qualify for the rollback function
    if(period %in% "daily")
      {period_return_tbl}
    else if(period %in% "weekly")
    {period_return_tbl}
    else 
    # rollback to first day of the month - ETF Issue ----
    # mutate(date = lubridate::rollback(date, roll_to_first = TRUE))  
    {period_return_tbl %>% mutate(date = lubridate::rollback(date, roll_to_first = TRUE))}
