...
# Creates a long format of stock returns based on 'one' ticker
single_portfolio <- function(symbols, end, start){
...
# Creates a long format of sotck returns based on 'multiple' tickers
multi_asset_portfolio <- function(symbols, end, start, wts_tbl){
...
fs::dir_create("01_Scripts_Flex")
dump(c("multi_asset_portfolio"),
     file = "01_Scripts_Flex/wealth_index.R")



# Create a portfolio [outdated]
...
# Create a portfolio of returns with adjusted name
portfolio_data_2 <- function(symbols, end, start, wts_tbl, col_rename){
  ...
  returns_port_tbl <- returns_reg_date_tbl %>% 
    tq_portfolio(assets_col = symbol,
                 ...
                 weights = wts_tbl,
                 rebalance_on = "years") %>% 
    add_column(symbol = col_rename, .before = 1) %>% 
    rename(monthly.returns = portfolio.returns)
...
test_1 <- portfolio_data_2(symbols, end, start, wts_tbl, "Benchmark")
